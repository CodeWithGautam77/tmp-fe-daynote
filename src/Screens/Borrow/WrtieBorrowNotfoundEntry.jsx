...
import { updateBorrowEntry } from "../../apis/borrowSlice";

export default function WrtieBorrowNotfoundEntry({ forceData, setForceData }) {
  const dispatch = useDispatch();

  const { loggedIn } = useSelector((state) => state.authData);
  const { creditEntitys, debitEntitys, entLoading } = useSelector((state) => state.entData);
                      ...
                      <>
                        {selectEnttityArray.map((item, index) => (
                          <option
                            ...
                            ref={(el) => {
                              if (el && index === selectEnttityArray.length - 1) {
