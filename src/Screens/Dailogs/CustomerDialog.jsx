...
export default function CustomerDialog(props) {
  ...
  useEffect(() => {
    if (openCustomerDialog[1]) {
      setOldValues(openCustomerDialog[1]);
  ...
  useEffect(() => {
    if (!openCustomerDialog[1] && formik.values.name) {
      setOldValues((old) => ({
        ...old,
        name: `${formik.values.name} ${selectedSuggestedText}`,
      }));
    }
    if (openCustomerDialog[1] && formik.values.name && selectedSuggestedText) {
    ...
    <Fragment>
      ...
      >
        <DialogTitle className="modalHeader">
          <h3>{openCustomerDialog[1] ? "Update" : "Add"} Customer</h3>
        </DialogTitle>
